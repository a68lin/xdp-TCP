...
void create_ip_header(struct iphdr *ip_hdr) {
    ...
    ip_hdr->saddr = inet_addr("10.30.0.1");
	printf("IP address 10.30.0.1 in network byte order: 0x%x\n", ip_hdr->saddr);
    ip_hdr->daddr = inet_addr("10.30.0.4");
	fid.sender_ip = inet_addr("10.30.0.1");
	fid.receiver_ip = inet_addr("10.30.0.4");
	printf("IPs: %u, %u\n", fid.sender_ip, fid.receiver_ip);
}

void create_tcp_header(struct tcphdr *tcp_hdr) {
	memset(tcp_hdr, 0, sizeof(struct tcphdr));
	tcp_hdr->source = htons(230);
	tcp_hdr->dest = htons(231);
	tcp_hdr->doff = 5;
	tcp_hdr->ack_seq = htonl(10);

	fid.sender_port = htons(230);
	fid.receiver_port = htons(231);
	printf("Ports: 230, 231\n");
	//tcp_hdr->seq = htonl(0);
